...
namespace Constellation
{
    public class SquackConstellationNames : MonoBehaviour
    {
        ...
        [SerializeField] TextAsset posNameFile;
        [SerializeField] GameObject textPrefab;
        [SerializeField] Camera targetCamera;
        [SerializeField] float distance = 99f;
        [SerializeField] LanguageType m_languageType = LanguageType.Latin;
        ...
        void Start()
        {
            Debug.Log($"Start Names - {StarsDrawMode}");
        ...
        void DoShowOnlyVirgo()
        {
            ...
            foreach (GameObject go in m_nameObjList)
            {
                go.SetActive(StarsDrawMode switch
                {
                    0 => false,
                    1 => go.name.Contains("Virgo"),
                    2 => Utilities.ConstLongNames.Contains(go.name),
                    _ => true
                });
            }
        }
        ...
        public static List<NamePosData> CreateNamePosList(TextAsset _asset)
        {
            ...
            while (sr.Peek() > -1)
            {
                ...
                if (string.IsNullOrWhiteSpace(lineStr) || lineStr.TrimStart().StartsWith("//"))
                {
                    continue;
                }
                ...
                string[] secondNameArr = new string[]
                    { dataArr[5], dataArr[6], dataArr[7], dataArr[8] }; //, (dataArr[9] ?? dataArr[6]) };
