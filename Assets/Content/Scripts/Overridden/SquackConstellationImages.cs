        ...
        void ChangeStarsDrawMode()
        {
            ...
            foreach (GameObject go in m_imageObjList)
            {
                bool shouldDraw = StarsDrawMode switch
                {
                    ...
                    2 =>Utilities.ConstLongNames.Any(constName => go.name == $"local_{constName}") ,
                    _ => StarsDrawMode != 0
                };
                go.SetActive(shouldDraw);
            }
        }

        [SerializeField, HideInInspector] Hipparcos m_hipparcosScr;
        [SerializeField] GameObject m_imgPrefab;
        [SerializeField] Sprite[] m_zodiacImgArr;
        [SerializeField] ConstellationImagesSO m_imageInfoSO;
        List<GameObject> m_imageObjList;

        // Start is called before the first frame update
        void Start()
        {
            Debug.Log($"Start Images - {StarsDrawMode}");
