...
namespace Constellation
{
    public class SquackConstellationStars : MonoBehaviour
    {
        ...
        [SerializeField] Texture2D m_starTex;

        [SerializeField, Range(0.1f, 10f)] float m_scale = 1f;
        ...
        void Start()
        {
            Debug.Log($"Start Stars - {StarsDrawMode}");
            ...
            if (hipparcosScr.hipAppendList != null)
            {
                ...
                m_ps = SetEfcStars(m_hipList, transform, m_scale, hipparcosScr.distance, m_starTex, useParallax,
                    StarsDrawMode);
            }
        ...
        void Update()
        {
            if (m_hipList == null && hipparcosScr.hipAppendList != null)
            {
                ...
                m_hipList = StarsDrawMode switch
                {
                    1 => m_hipList.Where(x => Utilities.VirgoStars.Contains(x.hipId)).ToList(),
                    2 => m_hipList.Where(x => Utilities.ZodiacStars.Contains(x.hipId)).ToList(),
                    _ => m_hipList
                };
                m_ps = SetEfcStars(m_hipList, transform, m_scale, hipparcosScr.distance, m_starTex, useParallax,
                    StarsDrawMode);
        ...
        public static void SetMeshStars(List<Hipparcos.HipData> _hipList, Transform _meshTr, Material _meshMat,
            float _distance, bool _useParallax)
        {
            if (_meshTr != null)
            {
        ...
        static ParticleSystem SetEfcStars(
            List<Hipparcos.HipData> _hipList, Transform _paricleTr, float _scale, float _distance, Texture2D _efcTex,
            bool _useParallax,
            int starsDrawMode
        )
        {
            _hipList = starsDrawMode switch
            {
                1 => _hipList.Where(h => Utilities.VirgoStars.Contains(h.hipId)).ToList(),
                2 => _hipList.Where(h => Utilities.ZodiacStars.Contains(h.hipId)).ToList(),
                _ => _hipList
            };
