...
#region Others
    ...
    List<AsyncOperationHandle<GameObject>> _promises;
    ...
    void Update() {
        if (_isSimulating) {
            ...
            int simSecondsPerSecond = SimSecondsPerSecond(_simSliderField.Value);
    ...
    void ToggleSimulation() {
        ...
        main.startLifetime = SimSecondsPerSecond(OneDay) / SimSecondsPerSecond(_simSliderField.Value);
        if (_isSimulating)
            _lightParticle.GetComponent<ParticleSystem>().Play();
        else
            _lightParticle.GetComponent<ParticleSystem>().Stop();
    }
